...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/06/20 22:01:04 by ahwang            #+#    #+#             */
/*   Updated: 2026/06/20 22:01:05 by ahwang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	rotate_plane_x_axis(t_data *data, int flag, int i, int key)
{
	...
	vec = data->scene_ori->plane[i]->xyz_vec;
	data->scene_ori->plane[i]->xyz_vec
		= vec3(1 * vec.x + 0 * vec.y + 0 * vec.z,
			0 * vec.x + v_cos * vec.y + -1 * v_sin * vec.z,
			0 * vec.x + v_sin * vec.y + v_cos * vec.z);
...
void	rotate_plane(int key, t_data *data, int i)
{
	if (data->axis == AXIS_X && (key == KEY_UP || key == KEY_DOWN))
		rotate_plane_x_axis(data, PL_ROTATE_X, i, key);
	if (data->axis == AXIS_Y && (key == KEY_UP || key == KEY_DOWN))
		rotate_plane_y_axis(data, PL_ROTATE_Y, i, key);
	if (data->axis == AXIS_Z && (key == KEY_UP || key == KEY_DOWN))
		rotate_plane_z_axis(data, PL_ROTATE_Z, i, key);
}
