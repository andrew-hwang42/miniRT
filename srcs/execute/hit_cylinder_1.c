...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/06/20 22:02:48 by ahwang            #+#    #+#             */
/*   Updated: 2026/06/20 22:02:49 by ahwang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	is_cylinder_body(t_data *data, int cy_i, double distance)
{
	...
	norminette_is_cylinder_body(data, cy_i, d, cy_center);
	...
	if (vec3_dot_vec3(vec3_sub_vec3(vec3_add_vec3(data->ray->point,
					vec3_mul_scalar(data->ray->direc, t_body)),
...
t_vec3	calc_normal_vec_cylinder_body(t_data *data, t_hit *hit, int cy_i)
{
	...
	proj_c_hit_p = vec3_mul_scalar(data->obj[cy_i]->xyz_vec,
...
int	hit_cylinder(t_data *data, t_hit *hit, int cy_i, double distance)
{
	...
	if (data->obj[cy_i]->obj_type != CYLINDER)
		return (1);
	t_body = is_cylinder_body(data, cy_i, distance);
	t_circle = is_cylinder_circle(data, cy_i, distance);
