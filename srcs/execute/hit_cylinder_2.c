...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/06/20 22:02:45 by ahwang            #+#    #+#             */
/*   Updated: 2026/06/20 22:02:45 by ahwang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
				...
				double *t_circle, t_vec3 *hit_p)
{
	hit_p[TOP] = vec3_add_vec3(data->ray->point,
			vec3_mul_scalar(data->ray->direc, t_circle[TOP]));
	hit_p[BOTTOM] = vec3_add_vec3(data->ray->point,
			vec3_mul_scalar(data->ray->direc, t_circle[BOTTOM]));
}
...
double	is_cylinder_circle(t_data *data, int cy_i, double distance)
{
	...
	is_cylinder_circle_1(data, cy_i, cy_center, tc);
	...
	is_cylinder_circle_2(dot, t_circle);
	...
	is_cylinder_circle_3(data, t_circle, hit_p);
